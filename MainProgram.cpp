...
bool isSameSize(const Rectangle& r1, const Rectangle& r2) {
    // İki dikdörtgenin alanları eşitse (veya boyutları birebir tutuyorsa) true döner
...
int main() {
    ...
    demo.doubleValue(); // Değeri mutasyona uğratır
    std::cout << "After doubleValue(): " << demo.getValue() << std::endl;
    const ConstDemo constDemo(7);
    std::cout << "\nConst obj initial value: " << constDemo.getValue() << std::endl;
    std::cout << "Const obj constGetDouble() result: " << constDemo.constGetDouble() << std::endl;
    std::cout << "Const obj value remains unchanged: " << constDemo.getValue() << std::endl;
    return 0;
}
